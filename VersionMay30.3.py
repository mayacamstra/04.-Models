...
def read_and_preprocess_data(path):
    ...
    data = data.T  # Transpose the data to have variables as columns
    return data
...
def perform_pca(data, n_components=9):
    ...
    return pd.DataFrame(pca_components, index=data.index, columns=[f'PC{i+1}' for i in range(n_components)])
...
data_static = read_and_preprocess_data(path_static)
data_forward = read_and_preprocess_data(path_forward_looking)

# Data standardizing
scaled_data_static = standardize_data(data_static)
scaled_data_forward = standardize_data(data_forward)

# Merge into combined data frame
combined_df = pd.concat([scaled_data_static, scaled_data_forward])

# Apply Hodrick-Prescott filter
filtered_df_combined = apply_hpfilter(combined_df)
filtered_df_static = apply_hpfilter(scaled_data_static)
...
def plot_pca_components(pca_df_combined, pca_df_static):
    ...
    for i in range(1, 5):
        plt.subplot(4, 1, i)
        plt.plot(pca_df_combined.index, pca_df_combined[f'PC{i}'], label='Combined Data')
        plt.plot(pca_df_static.index, pca_df_static[f'PC{i}'], label='Static Data', linestyle='--')
        plt.xlabel('Time')
...
# Uncomment the following line to plot PCA components
