...
# Behoud originele kolommen TEST
original_columns = scaled_df_static.columns
...
class DynamicFactorModel:
    def __init__(self, df_data: pd.DataFrame, num_factors: int, timescale: int):
        ...
        self.original_columns = df_data.columns  # Save original columns
        self.detrended_data = self.detrend_with_moving_average(self.df_data)
        self.detrended_columns = self.detrended_data.columns  # Save detrended columns
        self.detrended_data = self.detrended_data.dropna(axis=1, how = 'any')  # Remove NaN values
        self.std_data = self.standardize_df(self.detrended_data)  # Standardize the detrended data
        self.standardized_columns = self.std_data.columns  # Save standardized columns
        self.std_data = self.std_data.dropna(axis=1, how = 'any')  # Remove NaN values after standardization
        ...
        removed_after_detrending = set(self.original_columns) - set(self.detrended_columns)
        removed_after_standardizing = set(self.detrended_columns) - set(self.standardized_columns)
        print("Columns removed after detrending:", removed_after_detrending)
        print("Columns removed after standardizing:", removed_after_standardizing)

    def detrend_with_moving_average(self, df, window=12):
        detrended = df - df.T.rolling(window=window).mean().T  # Apply rolling mean along columns
        detrended = detrended.dropna(axis=1, how = 'any') # Verwijder kolommen met NaN-waarden
        if detrended.isnull().values.any():
            print("NaN values found after detrending.")
        return detrended

    def standardize_df(self, df):
        standardized = (df - df.mean(axis=1).values.reshape(-1,1)) / df.std(axis=1).values.reshape(-1,1)
        standardized = standardized.dropna(axis=1, how = 'any') # Verwijder kolommen met NaN-waarden
        if standardized.isnull().values.any():
            print("NaN values found after standardizing.")
        return standardized
