...
# Debug statement: Print data na het inladen
print("\n--- Data after loading ---")
print(df_data.head())
...
# Debug statement: Print data na het filteren
print("\n--- Data after filtering ---")
print(filtered_df.head())
print(f"Shape of filtered_df: {filtered_df.shape}")

# Standardize the entire filtered dataset
...
try:
    ...
    validate_end_index = filtered_df.columns.get_loc(DATE_VALIDATE_END)

    # Debug statement: Print de indexen voor train en validation splits
    print(f"\nTraining data ends at index: {train_end_index}")
    print(f"Validation data starts at index: {validate_start_index}, ends at index: {validate_end_index}")
    
    # Split de gestandaardiseerde data in training en validatie sets
    Y_train_std = filtered_df_std.iloc[:, :train_end_index + 1]
    Y_validate_std = filtered_df_std.iloc[:, validate_start_index:validate_end_index + 1]
    
    # Debug statement: Print splits van de gestandaardiseerde data
    print("\n--- Y_train_std (first 5 rows/columns) ---")
    print(Y_train_std.iloc[:5, :5])
    print(f"Shape of Y_train_std: {Y_train_std.shape}")
    
    print("\n--- Y_validate_std (first 5 rows/columns) ---")
    print(Y_validate_std.iloc[:5, :5])
...
for num_factors in factor_range:
    ...
    current_index = list(Y_train_std.columns)
    ...
    # Check if the mean of the residuals (\\mu(\\varepsilon_{t})) is zero
    ...
    if np.allclose(residuals_mean, 0, atol=1e-5):
        print(f"No shift detected in the factors (\\mu(\\varepsilon_{t}) = 0) for {num_factors} factors.")
    else:
        print(f"Warning: Shift detected in the factors (\\mu(\\varepsilon_{t}) != 0) for {num_factors} factors.")
