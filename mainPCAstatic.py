...
for num_factors in factor_range:
	...
	# Voeg de voorspelde waarden voor 't+38' toe aan de trainingsdata
    extended_train_data_38 = np.hstack((extended_train_data, predicted_variables_t38.T))
    extended_train_data_38_std = standardize(extended_train_data_38.T).T
    extended_index_38 = extended_index + [next_timestamp_38]
    extended_train_df_38 = pd.DataFrame(extended_train_data_38_std, index=Y_train.index, columns=extended_index_38)
    model = DynamicFactorModel(extended_train_df_38, num_factors)
    model.std_data = extended_train_data_38_std.T
    model.apply_pca()
    model.yw_estimation()
    fac_train_extended_38 = model.factors.T
    data_train_extended_38 = extended_train_data_38_std.T
    print("Training extended model for t+39 with data and factors...")
    model.enet_fit(data_train_extended_38, fac_train_extended_38)
    if model.model_ena is None:
        raise ValueError("ElasticNet model is not set after fitting. Check enet_fit method.")
    next_timestamp_39 = next_timestamp_38 + 1
    next_timestamp_39_str = next_timestamp_39.strftime('%Y-%m')
    factor_forecast_39 = model.factor_forecast(next_timestamp_39_str, scenarios=1)
    if factor_forecast_39.shape[1] != num_factors:
        raise ValueError(f"Expected {num_factors} features, got {factor_forecast_39.shape[1]} features")
    predicted_factors_dict[num_factors] = np.hstack((predicted_factors_dict[num_factors], factor_forecast_39.T))
    predicted_variables_t39 = model.enet_predict(factor_forecast_39.reshape(1, -1))
	...
	# Voeg de voorspelde waarden voor 't+40' toe aan de trainingsdata
    extended_train_data_40 = np.hstack((extended_train_data, predicted_variables_t40.T))
    extended_train_data_40_std = standardize(extended_train_data_40.T).T
    extended_index_40 = extended_index + [next_timestamp_40]
    extended_train_df_40 = pd.DataFrame(extended_train_data_40_std, index=Y_train.index, columns=extended_index_40)
    model = DynamicFactorModel(extended_train_df_40, num_factors)
    model.std_data = extended_train_data_40_std.T
    model.apply_pca()
    model.yw_estimation()
    fac_train_extended_40 = model.factors.T
    data_train_extended_40 = extended_train_data_40_std.T
    print("Training extended model for t+41 with data and factors...")
    model.enet_fit(data_train_extended_40, fac_train_extended_40)
    if model.model_ena is None:
        raise ValueError("ElasticNet model is not set after fitting. Check enet_fit method.")
    next_timestamp_41 = next_timestamp_40 + 1
    next_timestamp_41_str = next_timestamp_41.strftime('%Y-%m')
    factor_forecast_41 = model.factor_forecast(next_timestamp_41_str, scenarios=1)
    if factor_forecast_41.shape[1] != num_factors:
        raise ValueError(f"Expected {num_factors} features, got {factor_forecast_41.shape[1]} features")
    predicted_factors_dict[num_factors] = np.hstack((predicted_factors_dict[num_factors], factor_forecast_41.T))
    predicted_variables_t41 = model.enet_predict(factor_forecast_41.reshape(1, -1))
    predicted_variables_dict[num_factors] = np.hstack((predicted_variables_dict[num_factors], predicted_variables_t41.T))
    # print(f"Predicted variables for {next_timestamp_41_str}:\n", predicted_variables_t41)

	# Voeg de voorspelde waarden voor 't+41' toe aan de trainingsdata
    extended_train_data_41 = np.hstack((extended_train_data, predicted_variables_t41.T))
    extended_train_data_41_std = standardize(extended_train_data_41.T).T
    extended_index_41 = extended_index + [next_timestamp_41]
    extended_train_df_41 = pd.DataFrame(extended_train_data_41_std, index=Y_train.index, columns=extended_index_41)
    model = DynamicFactorModel(extended_train_df_41, num_factors)
    model.std_data = extended_train_data_41_std.T
    model.apply_pca()
    model.yw_estimation()
    fac_train_extended_41 = model.factors.T
    data_train_extended_41 = extended_train_data_41_std.T
    print("Training extended model for t+42 with data and factors...")
    model.enet_fit(data_train_extended_41, fac_train_extended_41)
    if model.model_ena is None:
        raise ValueError("ElasticNet model is not set after fitting. Check enet_fit method.")
    next_timestamp_42 = next_timestamp_41 + 1
    next_timestamp_42_str = next_timestamp_42.strftime('%Y-%m')
    factor_forecast_42 = model.factor_forecast(next_timestamp_42_str, scenarios=1)
    if factor_forecast_42.shape[1] != num_factors:
        raise ValueError(f"Expected {num_factors} features, got {factor_forecast_42.shape[1]} features")
    predicted_factors_dict[num_factors] = np.hstack((predicted_factors_dict[num_factors], factor_forecast_42.T))
    predicted_variables_t42 = model.enet_predict(factor_forecast_42.reshape(1, -1))
    predicted_variables_dict[num_factors] = np.hstack((predicted_variables_dict[num_factors], predicted_variables_t42.T))
    # print(f"Predicted variables for {next_timestamp_42_str}:\n", predicted_variables_t42)

	# Voeg de voorspelde waarden voor 't+42' toe aan de trainingsdata
    extended_train_data_42 = np.hstack((extended_train_data, predicted_variables_t42.T))
    extended_train_data_42_std = standardize(extended_train_data_42.T).T
    extended_index_42 = extended_index + [next_timestamp_42]
    extended_train_df_42 = pd.DataFrame(extended_train_data_42_std, index=Y_train.index, columns=extended_index_42)
    model = DynamicFactorModel(extended_train_df_42, num_factors)
    model.std_data = extended_train_data_42_std.T
    model.apply_pca()
    model.yw_estimation()
    fac_train_extended_42 = model.factors.T
    data_train_extended_42 = extended_train_data_42_std.T
    print("Training extended model for t+43 with data and factors...")
    model.enet_fit(data_train_extended_42, fac_train_extended_42)
    if model.model_ena is None:
        raise ValueError("ElasticNet model is not set after fitting. Check enet_fit method.")
    next_timestamp_43 = next_timestamp_42 + 1
    next_timestamp_43_str = next_timestamp_43.strftime('%Y-%m')
    factor_forecast_43 = model.factor_forecast(next_timestamp_43_str, scenarios=1)
    if factor_forecast_43.shape[1] != num_factors:
        raise ValueError(f"Expected {num_factors} features, got {factor_forecast_43.shape[1]} features")
    predicted_factors_dict[num_factors] = np.hstack((predicted_factors_dict[num_factors], factor_forecast_43.T))
    predicted_variables_t43 = model.enet_predict(factor_forecast_43.reshape(1, -1))
    predicted_variables_dict[num_factors] = np.hstack((predicted_variables_dict[num_factors], predicted_variables_t43.T))
    # print(f"Predicted variables for {next_timestamp_43_str}:\n", predicted_variables_t43)

	# Voeg de voorspelde waarden voor 't+43' toe aan de trainingsdata
    extended_train_data_43 = np.hstack((extended_train_data, predicted_variables_t43.T))
    extended_train_data_43_std = standardize(extended_train_data_43.T).T
    extended_index_43 = extended_index + [next_timestamp_43]
    extended_train_df_43 = pd.DataFrame(extended_train_data_43_std, index=Y_train.index, columns=extended_index_43)
    model = DynamicFactorModel(extended_train_df_43, num_factors)
    model.std_data = extended_train_data_43_std.T
    model.apply_pca()
    model.yw_estimation()
    fac_train_extended_43 = model.factors.T
    data_train_extended_43 = extended_train_data_43_std.T
    print("Training extended model for t+44 with data and factors...")
    model.enet_fit(data_train_extended_43, fac_train_extended_43)
    if model.model_ena is None:
        raise ValueError("ElasticNet model is not set after fitting. Check enet_fit method.")
    next_timestamp_44 = next_timestamp_43 + 1
    next_timestamp_44_str = next_timestamp_44.strftime('%Y-%m')
    factor_forecast_44 = model.factor_forecast(next_timestamp_44_str, scenarios=1)
    if factor_forecast_44.shape[1] != num_factors:
        raise ValueError(f"Expected {num_factors} features, got {factor_forecast_44.shape[1]} features")
    predicted_factors_dict[num_factors] = np.hstack((predicted_factors_dict[num_factors], factor_forecast_44.T))
    predicted_variables_t44 = model.enet_predict(factor_forecast_44.reshape(1, -1))
    predicted_variables_dict[num_factors] = np.hstack((predicted_variables_dict[num_factors], predicted_variables_t44.T))
    # print(f"Predicted variables for {next_timestamp_44_str}:\n", predicted_variables_t44)

	# Voeg de voorspelde waarden voor 't+44' toe aan de trainingsdata
    extended_train_data_44 = np.hstack((extended_train_data, predicted_variables_t44.T))
    extended_train_data_44_std = standardize(extended_train_data_44.T).T
    extended_index_44 = extended_index + [next_timestamp_44]
    extended_train_df_44 = pd.DataFrame(extended_train_data_44_std, index=Y_train.index, columns=extended_index_44)
    model = DynamicFactorModel(extended_train_df_44, num_factors)
    model.std_data = extended_train_data_44_std.T
    model.apply_pca()
    model.yw_estimation()
    fac_train_extended_44 = model.factors.T
    data_train_extended_44 = extended_train_data_44_std.T
    print("Training extended model for t+45 with data and factors...")
    model.enet_fit(data_train_extended_44, fac_train_extended_44)
    if model.model_ena is None:
        raise ValueError("ElasticNet model is not set after fitting. Check enet_fit method.")
    next_timestamp_45 = next_timestamp_44 + 1
    next_timestamp_45_str = next_timestamp_45.strftime('%Y-%m')
    factor_forecast_45 = model.factor_forecast(next_timestamp_45_str, scenarios=1)
    if factor_forecast_45.shape[1] != num_factors:
        raise ValueError(f"Expected {num_factors} features, got {factor_forecast_45.shape[1]} features")
    predicted_factors_dict[num_factors] = np.hstack((predicted_factors_dict[num_factors], factor_forecast_45.T))
    predicted_variables_t45 = model.enet_predict(factor_forecast_45.reshape(1, -1))
    predicted_variables_dict[num_factors] = np.hstack((predicted_variables_dict[num_factors], predicted_variables_t45.T))
    # print(f"Predicted variables for {next_timestamp_45_str}:\n", predicted_variables_t45)

	# Voeg de voorspelde waarden voor 't+45' toe aan de trainingsdata
    extended_train_data_45 = np.hstack((extended_train_data, predicted_variables_t45.T))
    extended_train_data_45_std = standardize(extended_train_data_45.T).T
    extended_index_45 = extended_index + [next_timestamp_45]
    extended_train_df_45 = pd.DataFrame(extended_train_data_45_std, index=Y_train.index, columns=extended_index_45)
    model = DynamicFactorModel(extended_train_df_45, num_factors)
    model.std_data = extended_train_data_45_std.T
    model.apply_pca()
    model.yw_estimation()
    fac_train_extended_45 = model.factors.T
    data_train_extended_45 = extended_train_data_45_std.T
    print("Training extended model for t+46 with data and factors...")
    model.enet_fit(data_train_extended_45, fac_train_extended_45)
    if model.model_ena is None:
        raise ValueError("ElasticNet model is not set after fitting. Check enet_fit method.")
    next_timestamp_46 = next_timestamp_45 + 1
    next_timestamp_46_str = next_timestamp_46.strftime('%Y-%m')
    factor_forecast_46 = model.factor_forecast(next_timestamp_46_str, scenarios=1)
    if factor_forecast_46.shape[1] != num_factors:
        raise ValueError(f"Expected {num_factors} features, got {factor_forecast_46.shape[1]} features")
    predicted_factors_dict[num_factors] = np.hstack((predicted_factors_dict[num_factors], factor_forecast_46.T))
    predicted_variables_t46 = model.enet_predict(factor_forecast_46.reshape(1, -1))
    predicted_variables_dict[num_factors] = np.hstack((predicted_variables_dict[num_factors], predicted_variables_t46.T))
    # print(f"Predicted variables for {next_timestamp_46_str}:\n", predicted_variables_t46)

	# Voeg de voorspelde waarden voor 't+46' toe aan de trainingsdata
    extended_train_data_46 = np.hstack((extended_train_data, predicted_variables_t46.T))
    extended_train_data_46_std = standardize(extended_train_data_46.T).T
    extended_index_46 = extended_index + [next_timestamp_46]
    extended_train_df_46 = pd.DataFrame(extended_train_data_46_std, index=Y_train.index, columns=extended_index_46)
    model = DynamicFactorModel(extended_train_df_46, num_factors)
    model.std_data = extended_train_data_46_std.T
    model.apply_pca()
    model.yw_estimation()
    fac_train_extended_46 = model.factors.T
    data_train_extended_46 = extended_train_data_46_std.T
    print("Training extended model for t+46 with data and factors...")
    model.enet_fit(data_train_extended_46, fac_train_extended_46)
    if model.model_ena is None:
        raise ValueError("ElasticNet model is not set after fitting. Check enet_fit method.")
    next_timestamp_47 = next_timestamp_46 + 1
    next_timestamp_47_str = next_timestamp_47.strftime('%Y-%m')
    factor_forecast_47 = model.factor_forecast(next_timestamp_47_str, scenarios=1)
    if factor_forecast_47.shape[1] != num_factors:
        raise ValueError(f"Expected {num_factors} features, got {factor_forecast_47.shape[1]} features")
    predicted_factors_dict[num_factors] = np.hstack((predicted_factors_dict[num_factors], factor_forecast_47.T))
    predicted_variables_t47 = model.enet_predict(factor_forecast_47.reshape(1, -1))
    predicted_variables_dict[num_factors] = np.hstack((predicted_variables_dict[num_factors], predicted_variables_t47.T))
    # print(f"Predicted variables for {next_timestamp_47_str}:\n", predicted_variables_t47)
