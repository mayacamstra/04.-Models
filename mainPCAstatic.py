...
forecast_steps = 3  # Set to 3 for initial testing
...
for num_factors in factor_range:
    ...
    # Set the initial timestamp for forecasting
    next_timestamp = '2020-01'  # Start with the first month after training period

    # Start with an index that contains the original Y_train columns
    extended_index = list(Y_train.columns)

    for step in range(1, forecast_steps + 1):
        print(f"Predicting step {step} ahead: {next_timestamp}")

        # Forecast factors for the current time step
        factor_forecast = model.factor_forecast(next_timestamp, scenarios=1)

        # Check if the forecasted factors have the correct shape
        ...
        # Add predicted factors to the matrix in the dictionary
        ...
        # Add the predicted variables to the matrix in the dictionary
        if num_factors not in predicted_variables_dict:
            predicted_variables_dict[num_factors] = predicted_variables.T
        else:
            predicted_variables_dict[num_factors] = np.hstack((predicted_variables_dict[num_factors], predicted_variables.T))

        print(f"Predicted variables for {next_timestamp}:\n", predicted_variables)

        # Extend training data with the predicted variables for the current step
        extended_train_data = np.hstack((Y_train_std, predicted_variables_dict[num_factors]))

        # Standardize the extended dataset
        extended_train_data_std = standardize(extended_train_data.T).T

        # Update the extended_index with the new timestamp
        extended_index.append(pd.Period(next_timestamp, freq='M'))

        # Converteer de uitgebreide gestandaardiseerde data terug naar een DataFrame
        extended_train_df = pd.DataFrame(extended_train_data_std, index=Y_train.index, columns=extended_index)

        # Update the model with the extended DataFrame
        model = DynamicFactorModel(extended_train_df, num_factors)
        model.std_data = extended_train_data_std.T
        model.apply_pca()
        model.yw_estimation()
        model.enet_fit(data_train, fac_train)  # Refit with the extended data

        # Update the next timestamp
        next_timestamp = (pd.Period(next_timestamp, freq='M') + 1).strftime('%Y-%m')

    # Store other results and perform residual plots as before
