...
class DynamicFactorModel:
    ...
    def detrend_with_moving_average(self, df, window=12):
        return df.sub(df.rolling(window=window, axis=1).mean(), axis=0)

    def standardize_df(self, df):
        return (df.T - df.mean(axis=1)).T / df.std(axis=1).T

    def apply_pca(self):
        pca = PCA(n_components=self.num_factors)
        principal_components = pca.fit_transform(self.std_data.T).T
        ...
        self.df_factors = self.standardize_df(df_factors.T).T
    ...
    def factor_forecast(self, future_date: str, scenarios: int = 100, ci_percentiles: tuple = (2.5, 97.5)) -> pd.DataFrame:
        ...
        forecast_length = pd.date_range(start=self.df_factors.columns[-1], end=future_date, freq='M').shape[0]
        ...
        forecast_index = pd.date_range(start=self.df_factors.columns[-1], periods=forecast_length, freq='M')
        forecast_df = pd.DataFrame(data=mean_forecast, index=forecast_index, columns=self.df_factors.index)
        for i, column in enumerate(self.df_factors.index):
    ...
    def predict_original_variables(self, factor_forecast_df):
        ...
        return original_var_forecast

def plot_original_variable_forecast(df, variable_name):
    if variable_name not in df.index:
        raise KeyError(f"Row '{variable_name}' not found in DataFrame.")
    
    plt.figure(figsize=(10, 6))
    plt.plot(df.columns, df.loc[variable_name], label=variable_name)
    plt.title(f"Forecast for {variable_name}")
    plt.xlabel("Time")
