...
# Zorg ervoor dat de directory bestaat waar we de resultaten en plots gaan opslaan
save_directory = r"C:\Thesis\04. Models\PLScombined"  # Map waar je de Excel-bestanden wil opslaan
os.makedirs(save_directory, exist_ok=True)

# Zorg ervoor dat de directory bestaat waar we de plots gaan opslaan
plot_dir = os.path.join(save_directory, "plots_PLScombined")
...
results_path = os.path.join(save_directory, 'results_PLScombined_with_AIC_BIC_AdjustedR2_LogLikelihood_Residuals.xlsx')
...
for num_factors, matrix in predicted_factors_dict.items():
    factors_path = os.path.join(save_directory, f'PLScombined_predicted_factors_matrix_{num_factors}.xlsx')
    pd.DataFrame(matrix).to_excel(factors_path, index=False)

for num_factors, matrix in predicted_variables_dict.items():
    variables_path = os.path.join(save_directory, f'PLScombined_predicted_variables_matrix_{num_factors}.xlsx')
